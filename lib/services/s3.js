...
AWS.util.update(AWS.S3.prototype, {
  ...
  willComputeChecksums: function willComputeChecksums(req) {
    console.log('[DEBUG] s3 willComputeChecksums called');
    ...
    // console.log('[DEBUG] rules.ContentMD5:', rules.ContentMD5);
    ...
    // console.log('[DEBUG] Signature version:', this.getSignatureVersion(req));
    ...
    if (needsContentMD5 && req.service.shouldDisableBodySigning(req) && !req.isPresigned()) {
      console.log('[DEBUG] s3 Returning true (sha256 signing disabled and not presigned)');
      return true;
    }

    // SigV2 and presign, for backwards compatibility purpose.
    if (needsContentMD5 && this.getSignatureVersion(req) === 's3' && req.isPresigned()) {
      console.log('[DEBUG] s3 Returning true (SigV2 and presigned)');
      return true;
    }
    console.log('[DEBUG] s3 Returning false (no checksum needed)');
    return false;
  },
  ...
  computeContentMd5: function computeContentMd5(req) {
    console.log('[DEBUG] computeContentMd5 called');
    console.log('[DEBUG] req.params:', req.params);
    // console.log('[DEBUG] req.httpRequest.body:', req.httpRequest.body);
    if (req.service.willComputeChecksums(req)) {
      var md5 = AWS.util.crypto.md5(req.httpRequest.body, 'base64');
      req.httpRequest.headers['Content-MD5'] = md5;
      console.log('[DEBUG] Computed MD5:', md5);
    } else {
      console.log('[DEBUG] willComputeChecksums returned false, MD5 not computed');
    }
  },
